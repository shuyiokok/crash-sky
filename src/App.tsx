    ...
    <div className="app-root">
      <div className="lobby">
        ...
        <p className="chip-balance">
          免费筹码 <strong>{Math.floor(game.balance).toLocaleString('en-US')}</strong>
        </p>
        <p className="lobby-hint">{gameConfig.meta.rtpHint}</p>
        <button type="button" className="open-crash" onClick={gate.openManual}>
          开始游戏
        </button>
        {gate.suppressToday && (
          <p className="suppress-hint">今日已勾选不再自动弹出，点上方按钮可手动进入</p>
        )}
